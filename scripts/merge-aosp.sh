...
function is_in_blacklist() {
  if [[ $( grep -i "caf" $WORKING_DIR/.repo/manifests/README.mkdn) ]]; then
...
function get_repos() {
  ...
  do
    if grep -q "$i" /tmp/rebase.tmp; then # If Google has it and
      if grep -q "$i" $WORKING_DIR/.repo/manifests/snippets/$MANIFEST; then # If we have it in our manifest and
        if grep "$i" $WORKING_DIR/.repo/manifests/snippets/$MANIFEST | grep -q "remote="; then # If we track our own copy of it
